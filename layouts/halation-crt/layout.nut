//
// Attract-Mode Front-End - "Halation Shader" sample layout
//
class UserConfig {
	</ label="CRT Geometry", help="Turn on/off CRT geometry in shader", options="on,off" />
	CRT_Geometry="off";
        </ label="Aperture Type", help="CRT Aperture Type", options="VGA,TV,Plain" />
	CRT_Aperture="Plain";
}
...
if  (layoutSettings["CRT_Geometry"]  == "on"){
    shader.set_param("CRT_geom", 1.0);
} else {
    shader.set_param("CRT_geom", 0.0);
}

switch (layoutSettings["CRT_Aperture"]){
        case "VGA": shader.set_param( "aperature_type", 0.0 );
            break;
        case "TV": shader.set_param( "aperature_type", 1.0 );
            break;
        case "Plain": shader.set_param( "aperature_type", 2.0 );
            break;
}
