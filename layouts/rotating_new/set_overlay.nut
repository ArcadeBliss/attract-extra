...
function overlayUpdate_Horz()
{
    ...
    overlay_horz.width = (overlay_horz.height * (overlay_horz.texture_width * 1.0 / overlay_horz.texture_height));
...
function overlayUpdate_Vert()
{
    ...
    // Set the width by scaling the original texture
    overlay_vert.height = (overlay_vert.width * (overlay_vert.texture_height * 1.0 / overlay_vert.texture_width));
    // Now we can set the X pos accurately. 
    // overlay_vert.width will return 0 if we didn't do the above.
    overlay_vert.x = (fe.layout.width / 2.0) - (overlay_vert.width / 2.0);
    overlay_vert.y = (fe.layout.height / 2.0) - (overlay_vert.height / 2.0);
